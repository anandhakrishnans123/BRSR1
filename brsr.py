...
if client_file is not None:
    ...
    st.write("Select columns for mapping (default values are pre-selected):")
    selected_columns = {}

    # Allow the user to select or change column mappings from default values
    for template_col, default_client_col in default_column_mapping.items():
        if default_client_col in df.columns:
            selected_columns[template_col] = st.selectbox(
                f'Select column for {template_col}', df.columns, index=df.columns.get_loc(default_client_col)
            )
        else:
            selected_columns[template_col] = st.selectbox(
                f'Select column for {template_col}', df.columns
            )
